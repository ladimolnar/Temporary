...
IF /I "%1"=="/ResetAll"    (SET TST_ResetAll=1&     GOTO LblNextCmdParameter1)
...
IF    "%TST_ResetAll%"=="1"    (CALL :SubExecuteResetAllVariables    & IF ERRORLEVEL 1 (GOTO LblFatalError) ELSE (GOTO :LblDone) )
...
ECHO To execute all the tests on the Quick Start database run:
ECHO     TST /RunAll TSTQuickStart 
...
REM   1 - An error occured. An error message was already displayed.
REM ==============================================================
:SubExecuteResetAllVariables

SET TST_ResetAllVariablesExitCode=0

sqlcmd %TST_SqlCmdParameters% -d TST -Q "EXEC Utils.ResetAllTSTVariables" > %TST_SqlOutput% 2>&1
IF ERRORLEVEL 1 (
   ECHO Error reseting the TST Variables.>&2
   ECHO See %TST_SqlOutput% >&2
   GOTO LblExecuteResetAllVariablesError
)

ECHO All TST variables were reset.

GOTO LblExecuteResetAllVariablesDone

:LblExecuteResetAllVariablesError
SET TST_ResetAllVariablesExitCode=1

:LblExecuteResetAllVariablesDone
EXIT /B %TST_ResetAllVariablesExitCode%
GOTO :EOF
...
IF "%TST_SetVarDatabaseName%"=="" (ECHO Invalid command line. The database name is not specified for the command /Set. >&2& GOTO LblExecuteSetVariableError)
IF "%TST_SetVarName%"==""         (ECHO Invalid command line. The TST variable name is not specified for the command /Set. >&2& GOTO LblExecuteSetVariableError)
IF "%TST_SetVarValue%"==""        (ECHO Invalid command line. The TST variable value is not specified for the command /Set. >&2& GOTO LblExecuteSetVariableError)
...
sqlcmd %TST_SqlCmdParameters% -d TST -Q "EXEC TST.Internal.PrintResults @TestSessionId=%TST_TSTTestSessionId%, @ResultsFormat='XML' %TST_SqlPrintParameters%" > %TST_SqlGenerateXmlOutput% 2>&1
IF ERRORLEVEL 1 (
...
ECHO                    [/UseEXIT]
ECHO.
ECHO TST run with no parameters is equivalent with:
ECHO TST /Setup /Server localhost
ECHO.
...
ECHO                 If the database 'TST' version %TST_MajorVersion%.%TST_MinorVersion% already exists 
...
ECHO  /ResetAll      Will reset all TST variables.
